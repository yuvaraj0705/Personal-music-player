...
import { Play, Pause, MoreVertical, Music, Heart, Volume2, Plus, Disc3 } from 'lucide-react';
...
export const HomeTab: React.FC<HomeTabProps> = ({ 
  ...
  // Safely map recently played IDs to actual tracks, fallback to first 3 tracks
  const recentTracks = recentlyPlayed
    .map(id => tracks.find(t => t.id === id))
    .filter((t): t is Track => t !== undefined)
    .slice(0, 5);
  
  const displayRecent = recentTracks.length > 0 ? recentTracks : tracks.slice(0, 3);
  const recentlyAdded = [...tracks].reverse().slice(0, 6);
    ...
    <div className="w-full h-full pb-36">
      ...
      <section className="mb-10">
        ...
        {displayRecent.length > 0 ? (
          ...
          <div className="text-sm text-brand-on-surface-variant flex items-center gap-2 bg-white/5 p-4 rounded-xl border border-white/5">
            <Disc3 size={16} /> No recent tracks found.
          </div>
        )}
      </section>
      ...
      <section className="mb-10">
        ...
        {playlists.length > 0 ? (
            ...
            {playlists.map((playlist, i) => {
              return (
                <motion.div 
                  ...
                  className="flex items-center gap-3.5 bg-brand-surface-container/30 hover:bg-brand-surface-container/60 backdrop-blur-md rounded-xl p-3 group transition-colors cursor-pointer border border-white/5 hover:border-white/10 shadow-sm"
                >
          ...
          <div className="bg-brand-surface-container/20 border border-white/5 rounded-2xl p-8 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-brand-on-surface font-semibold mb-1">No Playlists Yet</h3>
            <p className="text-sm text-brand-on-surface-variant max-w-[250px] mx-auto">
              Head over to your Library to create your first custom playlist.
            </p>
          </div>
        )}
      </section>
          ...
          {recentlyAdded.map((track, i) => {
            ...
            return (
              <motion.div 
                ...
                onClick={() => playTrack(track)}
                className="flex items-center justify-between p-3 rounded-xl hover:bg-brand-surface-container/40 backdrop-blur-sm transition-all group cursor-pointer border border-transparent hover:border-white/5"
              >
